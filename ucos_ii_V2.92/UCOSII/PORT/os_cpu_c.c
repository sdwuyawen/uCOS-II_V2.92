...
void  OSInitHookBegin (void)
{
	...
	/* OS_CPU_ExceptStk[OS_CPU_EXCEPT_STK_SIZE]已经越界
	 * M4是满递减栈，SP先自减，再入栈，所以使用堆栈时不会越界
	 * OS_CPU_ExceptStkBase是给MSP使用的。
	 * 在进入中断服务例程过程中，压栈时采用在线程模式使用的栈
	 * 在中断服务器程序中，PUSH和POP操作都使用MSP
	 * 在用BX LR中断返回时，若LR.2 == 1使用PSP出栈；若LR.2 == 0使用MSP出栈。
	 * 
	 */
...
OS_STK *OSTaskStkInit (void (*task)(void *p_arg), void *p_arg, OS_STK *ptos, INT16U opt)
{
	...
	/* 任务栈起始地址8字节对齐。这里并不是向下对齐8字节。
	 * 0000 -> 0000
	 * 0001 -> 0001
	 * ...
	 * 0111 -> 1000。由于M4属于FD栈（Full Decrease stack），SP指向栈顶元素。压栈时SP -= 4; *SP = val;
	 * 所以这里把0111对齐到了1000。即栈顶为1000。第一个压栈的元素压到0111地址。
	 */
	...
	//USE_FPU
    *(--p_stk) = (OS_STK)0x00000000uL;                          /* reserved                                             */
    *(--p_stk) = (OS_STK)0x00000000uL;                          /* FPSCR                                                */
    *(--p_stk) = (OS_STK)0x00000015uL;                          /* S15                                                  */
    *(--p_stk) = (OS_STK)0x00000014uL;                          /* S14                                                  */
    *(--p_stk) = (OS_STK)0x00000013uL;                          /* S13                                                  */
    *(--p_stk) = (OS_STK)0x00000012uL;                          /* S12                                                  */
    *(--p_stk) = (OS_STK)0x00000011uL;                          /* S11                                                  */
    *(--p_stk) = (OS_STK)0x00000010uL;                          /* S10                                                  */
    *(--p_stk) = (OS_STK)0x00000009uL;                          /* S9                                                   */
    *(--p_stk) = (OS_STK)0x00000008uL;                          /* S8                                                   */
    *(--p_stk) = (OS_STK)0x00000007uL;                          /* S7                                                   */
    *(--p_stk) = (OS_STK)0x00000006uL;                          /* S6                                                   */
    *(--p_stk) = (OS_STK)0x00000005uL;                          /* S5                                                   */
    *(--p_stk) = (OS_STK)0x00000004uL;                          /* S4                                                   */
    *(--p_stk) = (OS_STK)0x00000003uL;                          /* S3                                                   */
    *(--p_stk) = (OS_STK)0x00000002uL;                          /* S2                                                   */
    *(--p_stk) = (OS_STK)0x00000001uL;                          /* S1                                                   */
    *(--p_stk) = (OS_STK)0x00000000uL;                          /* S0                                                   */
    ...
    *(--p_stk) = (OS_STK)0x04040404uL;                          /* R4                                                   */
                                                                ...
                                                                /* Remaining registers of FPU saved on process stack    */
//USE_FPU
//     *(--p_stk) = (OS_STK)0x00000031uL;                          /* S31                                                  */
//     *(--p_stk) = (OS_STK)0x00000030uL;                          /* S30                                                  */
//     *(--p_stk) = (OS_STK)0x00000029uL;                          /* S29                                                  */
//     *(--p_stk) = (OS_STK)0x00000028uL;                          /* S28                                                  */
//     *(--p_stk) = (OS_STK)0x00000027uL;                          /* S27                                                  */
//     *(--p_stk) = (OS_STK)0x00000026uL;                          /* S26                                                  */
//     *(--p_stk) = (OS_STK)0x00000025uL;                          /* S25                                                  */
//     *(--p_stk) = (OS_STK)0x00000024uL;                          /* S24                                                  */
//     *(--p_stk) = (OS_STK)0x00000023uL;                          /* S23                                                  */
//     *(--p_stk) = (OS_STK)0x00000022uL;                          /* S22                                                  */
//     *(--p_stk) = (OS_STK)0x00000021uL;                          /* S21                                                  */
//     *(--p_stk) = (OS_STK)0x00000020uL;                          /* S20                                                  */
//     *(--p_stk) = (OS_STK)0x00000019uL;                          /* S19                                                  */
//     *(--p_stk) = (OS_STK)0x00000018uL;                          /* S18                                                  */
//     *(--p_stk) = (OS_STK)0x00000017uL;                          /* S17                                                  */
//     *(--p_stk) = (OS_STK)0x00000016uL;                          /* S16                                                  */
	
	/* 把LR的BIT4设置为1，表示该任务未使用过FPU，且初始的栈中不包含S16-S31 */
	/* 若把LR的BIT4设置为0，表示该任务使用过FPU，且初始的栈中必须包含S16-S31 */
