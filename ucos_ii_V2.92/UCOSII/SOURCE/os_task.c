...
/*$PAGE*/
	...
	/* 如果任务是就绪状态，则清除就绪表中原优先级的就绪状态，并设置新优先级的就绪状态 */
    ...
    if (pevent != (OS_EVENT *)0) {
		/* 清除事件等待列表中旧优先级的等待标志，并设置新优先级的等待标志 */
                     ...
                     INT8U    prio)
{
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        OS_EXIT_CRITICAL();
		/* 初始化任务栈，把任务代码指针存入任务栈中，返回栈底 */
        psp = OSTaskStkInit(task, p_arg, ptos, 0u);             /* Initialize the task's stack         */
		/* 初始化任务TCB，把栈底指针存入TCB中 */
...
/*$PAGE*/
    ...
    if (ptcb->OSTCBPrev == (OS_TCB *)0) {               /* Remove from TCB chain                       */
		/* 删除的是头节点 */
		/* 这里好像有问题，如果OSTCBList只有一个节点，那么ptcb->OSTCBNext就是空指针 */
...
INT8U  OSTaskResume (INT8U prio)
{
    ...
    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) != OS_STAT_RDY) { /* Task must be suspended                */
		...
		/* 清除挂起状态后，任务没有在等待sem,mbox,q,,mutex,flag,则任务进入就绪状态 */
        if (ptcb->OSTCBStat == OS_STAT_RDY) {                 /* See if task is now ready              */
			/* 不是一个等待任务，则使任务就绪，并调度运行 */
            if (ptcb->OSTCBDly == 0u) {
