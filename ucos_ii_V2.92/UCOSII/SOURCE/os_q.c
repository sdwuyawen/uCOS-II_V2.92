                      ...
                      INT16U    size)
{
    ...
    if (pevent != (OS_EVENT *)0) {               /* See if we have an event control block              */
        ...
        if (pq != (OS_Q *)0) {                   /* Were we able to get a queue control block ?        */
			...
			/* 没有获取到空的消息队列控制块，则释放事件控制块至空事件控制块链表 */
                   ...
                   INT8U     *perr)
{
    ...
    switch (opt) {
		...
		/* 不论是否有任务在等待消息，都删除消息队列 */
        case OS_DEL_ALWAYS:                                /* Always delete the queue                  */
			/* 使消息队列的事件等待列表中所有任务脱离等待状态
			 * 并设置任务控制块中脱离等待状态的原因是OS_STAT_PEND_ABORT
			 */
                ...
                void      *pmsg)
{
    ...
    OS_ENTER_CRITICAL();
	/* 如果事件等待列表非空，则消息队列空，直接把消息发送给等待任务 */
